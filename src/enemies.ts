...
function enemyBase(type: EnemyTier, x: number, y: number): Enemy {
  ...
  return {
    ...
    tier: type, color: '',
    canShoot: false, shootT: 80 + Math.random() * 60, shootInt: 100,
    stateT: 0,
...
export function mkEnemy(type: EnemyTier, x: number, y: number, floor: number): Enemy {
  ...
  switch (type) {
    ...
    case 'sniper':
      return { ...b, w: 22, h: 22, hp: 14 + f * 3, maxHp: 14 + f * 3, spd: 0.3, atk: 22, canShoot: true, shootInt: 150, color: '#1a3000', atkCdMax: 120, windup: 22, strikeDur: 6 };
    case 'splitter':
      return { ...b, w: 30, h: 30, hp: 38 + f * 6, maxHp: 38 + f * 6, spd: 0.6, atk: 9, canShoot: false, color: '#4a2800', atkCdMax: 100, windup: 20, strikeDur: 8 };
    case 'elite':
      return { ...b, w: 24, h: 24, hp: 48 + f * 10, maxHp: 48 + f * 10, spd: 1.4, atk: 17, canShoot: true, shootInt: 85, color: '#1a001a', atkCdMax: 70, windup: 14, strikeDur: 6, canBlock: true, guardHP: 3 };
    case 'minion':
      return { ...b, w: 18, h: 18, hp: 10, maxHp: 10, spd: 1.5, atk: 6, canShoot: false, color: '#1a1a1a', atkCdMax: 80, windup: 16, strikeDur: 8 };
    default:
      return { ...b, w: 22, h: 22, hp: 18 + f * 4, maxHp: 18 + f * 4, spd: 0.8, atk: 8, canShoot: f >= 3 && Math.random() < 0.35, shootInt: 100, color: '#2a0000' };
  }
}
...
export function makeBoss(floor: number, x: number, y: number): Enemy {
  ...
  return {
    ...
    spd: 1, atk: 22,
    canShoot: true, shootT: 55, shootInt: 50,
    color: '#991111',
    minionT: 220,
    aiState: 'chase',
