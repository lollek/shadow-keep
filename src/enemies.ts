...
export function spawnEnemy(x: number, y: number, floor: number): Enemy {
  ...
  if (floor >= 6 && r > 0.92) type = 'elite';
  else if (floor >= 3 && r < 0.18) type = 'sniper';
  else if (floor >= 2 && r < 0.38) type = 'charger';
...
export function mkEnemy(type: EnemyTier, x: number, y: number, floor: number): Enemy {
  ...
  switch (type) {
    case 'charger':
      return { ...b, w: 26, h: 26, hp: 28 + f * 5, maxHp: 28 + f * 5, spd: 1.8, atk: 13, meleeArc: 0.55, canShoot: false, color: '#8B0000', atkCdMax: 90, windup: 18, strikeDur: 8 };
    case 'sniper':
      return { ...b, w: 22, h: 22, hp: 14 + f * 3, maxHp: 14 + f * 3, spd: 0.3, atk: 22, meleeArc: 0.4, canShoot: true, shootT: 18, shootInt: 60, color: '#1a3000', atkCdMax: 120, windup: 22, strikeDur: 6 };
    case 'elite':
      return { ...b, w: 24, h: 24, hp: 48 + f * 10, maxHp: 48 + f * 10, spd: 1.4, atk: 17, meleeArc: 0.95, canShoot: true, shootT: 20, shootInt: 55, color: '#1a001a', atkCdMax: 70, windup: 14, strikeDur: 6, canBlock: true, guardHP: 3 };
    case 'minion':
      return { ...b, w: 18, h: 18, hp: 10, maxHp: 10, spd: 1.5, atk: 6, meleeArc: 0.82, canShoot: false, color: '#1a1a1a', atkCdMax: 80, windup: 16, strikeDur: 8 };
    default:
      return { ...b, w: 22, h: 22, hp: 18 + f * 4, maxHp: 18 + f * 4, spd: 0.8, atk: 8, meleeArc: 0.9, canShoot: f >= 3 && Math.random() < 0.35, shootT: 26, shootInt: 65, color: '#2a0000' };
  }
}
